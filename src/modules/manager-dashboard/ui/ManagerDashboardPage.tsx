...
export function ManagerDashboardPage() {
  ...
  return (
    <MayakShell scrollable contentClassName="grid gap-4 pb-8" topBar={<MayakTopBar brand="Valle Sanchez" subtitle="команда · учебный обзор" icon={<BarChart3 className="h-5 w-5" />} meta={<><UsersRound className="h-3.5 w-3.5 text-primary" />{records.length} сотрудников</>} userName="Руководитель" />}>
        ...
        <MayakPanel padding="md">
          ...
          <div className="mt-3 grid gap-2">
            {records.map((record) => {
              ...
              return <button className={`grid w-full cursor-pointer gap-2 rounded-[14px] border p-3 text-left transition hover:border-primary/45 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-ring/45 ${selected?.id === record.id ? "border-primary/50 bg-primary/5" : "border-border bg-card"}`} key={record.id} onClick={() => setSelectedId(record.id)} type="button">
...
function Metric({ label, value }: { label: string; value: number }) {
  return <MayakPanel padding="sm"><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">{label}</p><p className="mt-2 font-brand text-3xl font-medium text-foreground">{value}</p></MayakPanel>;
}
...
function EmployeeDetail({ record }: { record: ManagerDashboardRecord }) {
  return <MayakPanel padding="md" className="lg:sticky lg:top-24"><h2 className="font-brand text-2xl font-medium text-foreground">{record.name}</h2><p className="mt-1 text-xs text-muted-foreground">{record.location}</p><div className="mt-4 rounded-[14px] bg-secondary p-3"><p className="text-xs font-semibold text-foreground">Текущий этап</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{record.nextAction}</p></div><TopicList title="На этом сфокусирован маршрут" items={record.weakTopicTitles} empty="Только обязательные темы маршрута" /><TopicList title="Блокеры" items={record.blockedTaskTitles} empty="Открытых блокеров нет" /><TopicList title="Открытые вопросы наставнику" items={record.openEscalations} empty="Открытых вопросов нет" /><div className="mt-4 rounded-[14px] border border-border p-3"><p className="flex items-center gap-2 text-sm font-semibold text-foreground"><CheckCircle2 className="h-4 w-4 text-primary" />Как помочь</p><p className="mt-2 text-xs leading-relaxed text-muted-foreground">Свяжитесь с сотрудником, если задача заблокирована или нужен разбор темы. Решение о допуске к работе всегда остаётся за человеком.</p></div></MayakPanel>;
}
