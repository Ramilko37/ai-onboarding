...
const synonymTerms: Record<string, string[]> = {
  ...
  пролив: ["эспрессо", "помол", "экстракция", "водянистый", "время"],
  водянистый: ["быстрый", "пролив", "эспрессо", "помол"],
  быстрый: ["пролив", "эспрессо", "помол"],
  помол: ["доза", "трамбовка", "эспрессо", "экстракция", "кофемолка"],
  закладка: ["доза", "холдер", "весы", "грамм", "рецепт"],
  доза: ["закладка", "холдер", "весы", "грамм", "рецепт"],
  холдер: ["портафильтр", "рожок", "темпер", "темперовка", "закладка"],
  портафильтр: ["холдер", "рожок", "темпер", "темперовка"],
  темперовка: ["темпер", "трамбовка", "таблетка", "холдер"],
  рецепт: ["закладка", "экстракция", "выход", "эспрессо"],
  выход: ["рецепт", "эспрессо", "миллилитр", "38"],
  молоко: ["капучино", "латте", "микропена", "питчер", "текстура", "температура"],
  капучино: ["молоко", "микропена", "текстура", "температура"],
  латте: ["молоко", "микропена", "рецептура", "пена"],
  "флэт": ["уайт", "молоко", "эспрессо", "пена"],
  уайт: ["флэт", "молоко", "эспрессо", "пена"],
  температура: ["молоко", "капучино", "сладость", "пена"],
  температуры: ["температура", "молоко", "капучино", "сладость", "пена"],
  греть: ["температура", "молоко", "капучино"],
  нагревать: ["температура", "молоко", "капучино"],
...
function scoreChunk(params: {
  ...
  const titleSearchable = normalize(`${params.document.title} ${params.chunk.title}`);
  ...
  for (const term of params.queryTerms) {
    ...
    if (titleSearchable.includes(term)) {
