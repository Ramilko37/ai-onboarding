...
- 2026-07-21 welcome simplification: the separate right-hand presentation column was removed, the welcome card was constrained to 800 px without a fixed desktop height, the entry action was reduced to a single CTA (`Начать диагностику` or `Продолжить диагностику`) with a separate time/progress hint, and the development-only reset action moved into the profile menu.
